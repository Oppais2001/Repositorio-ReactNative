...
export default function App() {
  ...
  const MostrarPersonaje = () =>{
    ...
    if(Kurono.ComprobarIndice(Indice)){
      console.log("Kurono")
      if(Indice<12||(Indice>13&&Indice<43)||Indice==47||
      Indice==57||(Indice>59&&Indice<69)||
      (Indice>71&&Indice<83)){
  ...
  */
  ...
  useEffect(() => {
    (async () => {
      ...
      await KuronoUniforme.downloadAsync();
      await KuronoCamisa.downloadAsync();
      await KuronoTraje.downloadAsync();
      await KatoUniforme.downloadAsync();
      await KuronoSerio.downloadAsync();  
      await KuronoNervioso.downloadAsync();          
      await KuronoFeliz.downloadAsync(); 
      await KuronoSorprendido.downloadAsync();     
      await KatoSerio.downloadAsync();  
      await KatoNervioso.downloadAsync();          
      await KatoFeliz.downloadAsync(); 
      await KatoSorprendido.downloadAsync();        
  ...
  return(
    <View style={styles.container}>
      <Image source={{...Personaje}} style={styles.image} cachePolicy={'memory'} transition={1000}/>
      <Image source={{...Expresion}} style={styles.image} cachePolicy={'memory'} transition={1000}/>
      <TouchableOpacity style={styles.boton} onPress={MostrarPersonaje}>
